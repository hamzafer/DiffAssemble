...
# SINGLE-SIZE CONFIGURATION - UPDATED
PUZZLE_SIZES = [4]  # Only size 4
NUM_EXAMPLES_PER_SIZE = 3  # Changed from 1 to 3
MAX_ATTEMPTS = 200  # Increased to find more examples
CHECKPOINT_PATH = "/cluster/home/akmarala/DiffAssemble/Puzzle-Diff/hau6yh9s/checkpoints/last.ckpt"  # Updated checkpoint path

# Create output directory with meaningful name
output_dir = Path("results_texmet_4x4")
...
def create_image_from_patches(patches, pos, n_patches, rotations=None):
    ...
    for p in range(patches.shape[0]):
        ...
        # REMOVE rotation logic since no rotation
...
def analyze_puzzle_size(puzzle_size, model, device):
    ...
    print(f"Test dataset size: {len(test_dt)}")
    ...
    # 🆕 KEEP TRYING UNTIL WE FIND 3 SUCCESS AND 3 FAILURE EXAMPLES
    attempts = 0
    ...
    with torch.no_grad():
        while attempts < MAX_ATTEMPTS and len(tested_indices) < dataset_size:
            ...
            # REMOVE this block - your model expects 2D features, not 4D
            # if sample.x.shape[1] == 2:
            #     # Model expects 4D features (x, y, cos, sin) but sample only has 2D (x, y)
            #     dummy_rot = torch.tensor([[1.0, 0.0]]).repeat(sample.x.shape[0], 1)
            #     sample.x = torch.cat([sample.x, dummy_rot], dim=1)
            ...
            # REMOVE rotation accuracy calculation
            ...
            # 🆕 STOP WHEN WE HAVE 3 SUCCESS AND 3 FAILURE EXAMPLES
            if len(successes) >= NUM_EXAMPLES_PER_SIZE and len(failures) >= NUM_EXAMPLES_PER_SIZE:
                print(f"   🎯 Found {NUM_EXAMPLES_PER_SIZE} successes and {NUM_EXAMPLES_PER_SIZE} failures for {puzzle_size}x{puzzle_size}! Stopping search.")
                break
...
print(f"Examples per size: {NUM_EXAMPLES_PER_SIZE} (3 success + 3 failure)")  # Updated message
...
# Create consolidated SUCCESS figure - HORIZONTAL LAYOUT
print(f"\n🎯 Creating SUCCESS compilation...")
if all_successes:
    num_success = len(all_successes)
    fig_success, axes_success = plt.subplots(num_success, 3, figsize=(15, 5*num_success))
    ...
    for i, result in enumerate(all_successes):
        ...
        # Column 1: Scrambled
        axes_success[i, 0].imshow(result['scrambled_img'])
        axes_success[i, 0].set_title(f'Scrambled\n{size}x{size} Puzzle', fontsize=14, fontweight='bold')
        axes_success[i, 0].axis('off')
        
        # Column 2: Prediction
        axes_success[i, 1].imshow(result['pred_img'])
        axes_success[i, 1].set_title(f'SUCCESS\n{result["pieces_correct"]}/{result["total_pieces"]} pieces', 
                                   fontsize=14, fontweight='bold', color='green')
        axes_success[i, 1].axis('off')
        
        # Column 3: Ground Truth
        axes_success[i, 2].imshow(result['gt_img'])
        axes_success[i, 2].set_title(f'Ground Truth\n(Target)', fontsize=14, fontweight='bold')
        axes_success[i, 2].axis('off')
    ...
    success_path = output_dir / "success_texmet_4x4.png"
    plt.savefig(success_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"   💾 Saved: {success_path}")

# Create consolidated FAILURE figure - HORIZONTAL LAYOUT
print(f"\n❌ Creating FAILURE compilation...")
if all_failures:
    num_failure = len(all_failures)
    fig_failure, axes_failure = plt.subplots(num_failure, 3, figsize=(15, 5*num_failure))
    ...
    for i, result in enumerate(all_failures):
        ...
        # Column 1: Scrambled
        axes_failure[i, 0].imshow(result['scrambled_img'])
        axes_failure[i, 0].set_title(f'Scrambled\n{size}x{size} Puzzle', fontsize=14, fontweight='bold')
        axes_failure[i, 0].axis('off')
        
        # Column 2: Prediction
        axes_failure[i, 1].imshow(result['pred_img'])
        accuracy_pct = result['piece_accuracy'] * 100
        axes_failure[i, 1].set_title(f'FAILED\n{result["pieces_correct"]}/{result["total_pieces"]} pieces ({accuracy_pct:.1f}%)', 
                                   fontsize=14, fontweight='bold', color='red')
        axes_failure[i, 1].axis('off')
        
        # Column 3: Ground Truth
        axes_failure[i, 2].imshow(result['gt_img'])
        axes_failure[i, 2].set_title(f'Ground Truth\n(Target)', fontsize=14, fontweight='bold')
        axes_failure[i, 2].axis('off')
    ...
    failure_path = output_dir / "failure_texmet_4x4.png"
...
print(f"   📊 {success_path.name} - 1 success for 4x4 texmet")
print(f"   ❌ {failure_path.name} - 1 failure for 4x4 texmet")
